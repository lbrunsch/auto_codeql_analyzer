from common import plt, pd, np, Path, textwrap, RESULTATS_DIR, SQL_DIR, get_db, run_chart
...
def run(tab) :
	...
	ax=plt.gca()
	ax.margins(y=0)
	plt.xlim(0,df_plot["avg_occurrences_per_repo"].max() *1.2)
	plt.xticks(fontsize=12)
	plt.yticks(fontsize=12)
	plt.xlabel("Average occurrences per repo", fontweight="bold", fontsize=14)
	plt.ylabel("Green Issues types", fontweight="bold", fontsize=14)
